...
#define	SAVETARGA				13
		...
		void ImageModifier::treatLine( char *textLine) {
		...
		/// apply operators
		void ImageModifier::applyOper()
		{
			// get the image
			ImToOper = new GlobalImg( basestream::conform( getName()), 0 );
		...
		void ImageModifier::getLearn(char *mapName, int *posx, int *posy) {
		...
		void ImageModifier::resetGetLearn() {
			for (int i=0; i< nbOper; i++)
