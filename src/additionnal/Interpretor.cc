...
/*class Interpretor
{
	char *test;
public:*/
	void connect( istrngstream	*f ) { }
	void createMap( istrngstream	*f ) { }
	...
	void treatLineStr( char *lineur )
	{
		...
		while (f.eof() !=  EOF)
		{
			...
			// create a new map
			if (!strcasecmp( tmp1, "create_map")) {
				createMap( &f );
			}
			// connect two maps
			else if (!strcasecmp( tmp1, "connect")) {
				connect( &f );
			}
			// set a global variable
			else if (!strcasecmp( tmp1, "set")) {
				char tmp2[100];
				f >> tmp1;
				f >> tmp2;
			...
			// create a screen
			else if (!strcasecmp( tmp1, "create_screen")) {
			...
			// display a map
			else if (!strcasecmp( tmp1, "display")) {
