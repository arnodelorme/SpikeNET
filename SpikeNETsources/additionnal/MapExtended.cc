...
#define BORDER 5
#define IDEAL_DISCHARGE		5.0

MapExtended::carte(int numero0, OutputDisplay *disp0, char *name0, int largeur_norm0, int hauteur_norm0, double seuil0, char *fact_modulation0, int zoom0, double decay0, int convSpeed0) :
...
void carte::affiche()
{
	printf("%10s %4d\t(%d)%dx%d(%d)\t%6.2f\t%6.2f\t%d\t%.0f\t%d\t%d\n", name, numero, border_l, largeur_norm, hauteur_norm, border_h,
		 		(double) seuil, realDecay, 1 << zoom, map_SpikeInputs.compute_somme(), latencyReserv, convSpeed);
...
double carte::seuil_mod_dicot( double percent_seuil )
{
	...
	if (last_ordre_decharge < IDEAL_DISCHARGE)	seuil += (FORMAT_COMPUTE) ( percent_seuil* seuil * ( last_ordre_decharge - IDEAL_DISCHARGE )/IDEAL_DISCHARGE);
//	else								seuil += (FORMAT_COMPUTE) ( percent_seuil* seuil*( last_ordre_decharge/ideal_decharge));
	else								seuil += (FORMAT_COMPUTE) ( percent_seuil* seuil*log((last_ordre_decharge/IDEAL_DISCHARGE)*(last_ordre_decharge/IDEAL_DISCHARGE) + 1) );
...
void carte::init_neuron(GlobalImgTreat **listImg, int supervParam)
{
  int i,j,k;
  ...
  if (!convLockMap && learning == NULL)
  	 {
	 	learning    = new FORMAT_POIDS[2 * (EnvVar::MAX_TEMPS + MAX_COUCHE)];
  	 	...
  	 	for (int ii=0; ii < 2 * (EnvVar::MAX_TEMPS + MAX_COUCHE); ii++)
  	 		fprintf(saveLearn,  "%4.6f\n", learning [ ii ]);  	 	
  	 	fclose(saveLearn);
  	 	exit(-1);*/
  	 	
  	 	/*learning    = new FORMAT_POIDS[500];
  	 	learningPtr = learning +250;
  	 	
  	 	for (int i=-250; i < 250; i++)
   	 		if (i > 0)	learning[ i+250 ] = 0.0013*(FORMAT_POIDS)( exp( - (double)i / 250.0 )); //tau / orderModif));	//initialy 20.0
  	 		else		learning[ i+250 ] = 0.0003*(FORMAT_POIDS)( exp(    (double)i / 250.0 )); //tau / orderModif));	//initialy 20.0
  	 	*/
  ...
  // determination du premier neurone qui a decharge
/*  int first = 0;
...
#endif
	  {} else	if (nb_decharge_from [i + j*largeur_tot]) 
			if ((activite[i + j*largeur_tot] > activite[first]) || (first == 0)) 
				first = i + j*largeur_tot;*/
      ...
      else    
	  {
	  	...
	  	if ((!supervParam) || convLockMap) {	
	  		...
	  		nb_decharge_from	[pos] = 0;
	  	  	...
	  	  	if  (!nb_decharge_from	[pos]) //(pos != first)
	  	  		   for (k=0; k< map_SpikeInputs.nb_groups; k++) state[k][pos] = 0;
	  	  	else { for (k=0; k< map_SpikeInputs.nb_groups; k++) state[k][pos] = map_SpikeInputs.relevant_afferences[k];
	  	  			rebuild->addSpike( tab_x[pos], tab_y[pos], originalImg, 1.0);
	  	  	}
...
double carte::printState(double act, int t, int n, int s,double p,  double v) 
{
	if (!strcmp( name, "LEARN10"))
		printf("%10s\tAct:%4.2f\tt:%4d\tn:%7d\ts:%4d\tp:%4.2f\tv:%1.4f\n", name, act, t, n, s, p, v);
	return 0;
}
...
/*********************** dummy learning W += "1 or -1" */
		COMPUTE_MAP( (nb_decharge_from[tmp_neur] ? (*tmp_val = (( activite[tmp_neur] - tmp_state[tmp_neur]--) > 0) ? - 1 : + 1): 0 , 0) )
#endif
		
#if LEARNING_MODELE == 2
/*********************** order add : W += mod/alpha */
		COMPUTE_MAP( ((nb_decharge_from[tmp_neur] ? (*tmp_val = *tmp_val + (*(tab_modulation_tmp - tmp_state[tmp_neur]--)) / (FORMAT_POIDS)(convSpeed)) : 0), 0) )
#endif
...
#if LEARNING_MODELE == 26
/*********************** markram realadd : W = W + learning (saturation between 0 and 1000) */	
//	  	COMPUTE_MAP( ((nb_decharge_from[tmp_neur] ? (*tmp_val += (((temps - activite[tmp_neur]) <= 0) ? (1000 - *tmp_val)* *(learningPtr - tmp_state[tmp_neur]--) : - *tmp_val * *(learningPtr - tmp_state[tmp_neur]++))) : 0), 0) )
	  	COMPUTE_MAP( ((nb_decharge_from[tmp_neur] ? (*tmp_val += (((tmp_state[tmp_neur] - activite[tmp_neur]) >= 0) ? (1000 - *tmp_val)* *(learningPtr - activite[tmp_neur] + tmp_state[tmp_neur]--) : - *tmp_val * *(learningPtr - activite[tmp_neur] + tmp_state[tmp_neur]--))) : 0), 0) )
#endif

#if LEARNING_MODELE == 27  // asymétric, pas de modulation ++ these 3 ++
/*********************** markram pure add : W += ("0" or 1000") then renorm convolution*/
		COMPUTE_MAP( (nb_decharge_from[tmp_neur] ? (*tmp_val += (( activite[tmp_neur] - tmp_state[tmp_neur]--) <= 0) ? 1000 / convSpeed: 0): 0) )
#endif

#if LEARNING_MODELE == 27  // asymétric, pas de modulation ++ these 4 ++
/*********************** markram pure add : W += ("0" or 1000") then renorm convolution*/
		COMPUTE_MAP( (nb_decharge_from[tmp_neur] ? (*tmp_val += (( activite[tmp_neur] - tmp_state[tmp_neur]--) <= 0) ? 1000 / convSpeed: 0): 0) )

 //	  	COMPUTE_MAP( ((nb_decharge_from[tmp_neur] ? (*tmp_val += printState(activite[tmp_neur], temps, tmp_neur, tmp_state[tmp_neur]--, *tmp_val, *(learningPtr - activite[tmp_neur] + tmp_state[tmp_neur]) )) : 0), 0) )
//		COMPUTE_MAP( (nb_decharge_from[tmp_neur] ? (*tmp_val += (( activite[tmp_neur] - tmp_state[tmp_neur]--) <= 0) ? (1000- *tmp_val) * *(learningPtr + activite[tmp_neur] - tmp_state[tmp_neur]--): - *tmp_val * *(learningPtr + activite[tmp_neur] - tmp_state[tmp_neur]--)): 0) )
//		COMPUTE_MAP( (nb_decharge_from[tmp_neur] ? (*tmp_val += (( activite[tmp_neur] - tmp_state[tmp_neur]--) <= 0) ? 1000 * *(learningPtr + activite[tmp_neur] - tmp_state[tmp_neur]): 0): 0) )


#endif

#if LEARNING_MODELE == 242
/*********************** markram realconv 3 : W += ( "0 or 1000" - W )/(alpha*ndech) * learning */
		    COMPUTE_MAP( (nb_decharge_from[tmp_neur] ? (((activite[tmp_neur] - tmp_state[tmp_neur]) == 0) ? ((*tmp_val =  *tmp_val + ( 1000.0- *tmp_val) / (convSpeed*last_ordre_decharge) * *(learningPtr)),tmp_state[tmp_neur]--) : 0),
		    								(*tmp_val = (((activite[tmp_neur] - tmp_state[tmp_neur]) <= 0) ? 
		    									*tmp_val + (1000.0- *tmp_val) / (convSpeed*last_ordre_decharge) * *(learningPtr - tmp_state[tmp_neur]--) : 
		    									*tmp_val + (     0- *tmp_val) / (convSpeed*last_ordre_decharge) * *(learningPtr + tmp_state[tmp_neur]++))) : 0) )
#endif

#if LEARNING_MODELE == 252
/*********************** markram order lateral inhibition : warning bug within inhibtion because tmp_state reach 0 and then we cannot enter the test for inhibition */
		    COMPUTE_MAP( ( (nb_decharge_from[tmp_neur] ? (((activite[tmp_neur] - tmp_state[tmp_neur]) == 0) ? (tmp_spike2->x = tab_x[tmp_neur], tmp_spike2->y = tab_y[tmp_neur], tmp_spike2++, tmp_size++, tmp_state[tmp_neur]--) : 0) , 
		    							(*tmp_val = (((activite[tmp_neur] - tmp_state[tmp_neur]) <= 0) ? 
		    									*tmp_val + (((groupTreated == 0) ? (1000.0- *tmp_val) : 0) ) / ( convSpeed*last_ordre_decharge) * *(learningPtr + activite[tmp_neur] - tmp_state[tmp_neur]--) : 
		    									*tmp_val + (((groupTreated == 0) ? (0 - *tmp_val) : 0)) / (convSpeed*last_ordre_decharge)  * *(learningPtr + activite[tmp_neur] - tmp_state[tmp_neur]--))) : 0), 0) )
#endif
