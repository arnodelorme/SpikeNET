	...
	do
	  {
		...
		for (int i= begL; i< endL ; i++) {
		  ...
		  // local or remote map
		  proc *local = *(bewulf+ms_index*bewf*30) ? procManager->addTabProc( bewulf+ms_index*bewf*30) : (proc *) NULL;
		  if ((local != NULL)) // remote map
			...
			if ((learn[0] == 's') || (learn[0] == 'S')) {
			  SpikeNet[i]->addMap( tab_carte[nbMaps++]  = new carte_learn_superv( nbMaps, outputTmp, name, size_x, size_y, atof(seuil+ms_index*thrf*30) , mod+ms_index*modf*30, atoi(zoom) + scaling, atof(decay+ms_index*decf*30), atoi(conv)) );
			  EnvVar::LEARN_SUPERV = YES;
			}
			else if (mod[0] == 0)
			  if (decay[0] == 0)
				SpikeNet[i]->addMap( tab_carte[nbMaps++]  = new carte( nbMaps, outputTmp, name, size_x, size_y, atof(seuil+ms_index*thrf*30) , NULL, atoi(zoom) + scaling, atof(decay+ms_index*decf*30), atoi(conv)) );
			  else	SpikeNet[i]->addMap( tab_carte[nbMaps++]  = new carte_decay( nbMaps, outputTmp, name, size_x, size_y, atof(seuil+ms_index*thrf*30) , mod+ms_index*modf*30, atoi(zoom) + scaling, atof(decay+ms_index*decf*30), atoi(conv)) );
			else 
			  if (decay[0] == 0)
				SpikeNet[i]->addMap( tab_carte[nbMaps++]  = new carte_modul( nbMaps, outputTmp, name, size_x, size_y, atof(seuil+ms_index*thrf*30) , mod+ms_index*modf*30, atoi(zoom) + scaling, atof(decay+ms_index*decf*30), atoi(conv)) );
			  else	SpikeNet[i]->addMap( tab_carte[nbMaps++]  = new carte_modul_decay( nbMaps, outputTmp, name, size_x, size_y, atof(seuil+ms_index*thrf*30) , mod+ms_index*estf*30, atoi(zoom) + scaling, atof(decay+ms_index*decf*30), atoi(conv)) );
		  }				
			
		  //estimer
		  SpikeNet[i]->estimer->addMap( atoi(couche), tab_carte[nbMaps-1]->get_largeur_norm() * tab_carte[nbMaps-1]->get_hauteur_norm(), atof(estim+ms_index*estf*30) );
