...
void Split( char *file, int learn1run0)
{
	...
	if (!lastStr)	fprintf( fo, "image\t%s\t", scaling(tmp));
	else			fprintf( fo, "%s\t", scaling(tmp));
	while (f.eof() !=EOF) {
		f.getline( tmp, 500, '\n'); scaling(tmp);
		for (int ii=0; ii< nbLoop; ii++)
			if ((tmppos = strstr( tmp, learnMap[ii])) != NULL)
				if (!strcmp( tmp, learnMap[ii])) nbLearn[ii]++;
		...
		if ( strstr( tmp, "target") == NULL) fprintf(fo, "%s\n", tmp);
			...
			*tmp2 = 0;
			for (int i=0; i<nbLoop; i++)
				if (learn1run0)	fprintf( fo, "%s%s\tThreshold\t0.5\tFreq\t%d\tState\tSuperv\t%s\tPos\t%d\t%d\t0\n", tmp, learnMap[ i ], nbLearn[ i ], tmp2+6, (i/6)*123 + 530, (i %6)*120);
				else			fprintf( fo, "%s%s\tThreshold\t%f\tFreq\t%d\t%s\tPos\t%d\t%d\t0\n", tmp, learnMap[ i ], threshold[i], nbLearn[ i ], tmp2+6, (i/6)*123 + 530, (i %6)*120 );
	 		//break; //TO REMOVE
	 	}
	 }
	 fclose( fo);
